...
(function () {
    ...
    Game_Picture.prototype.show = function (name, origin, x, y, scaleX,
        ...
        if ($gameScreen.isSettingDText() && !name) {
            ...
            if (this.dTextInfo.isRuby) {
                arguments[2] -= RubyOffset;
                arguments[3] -= RubyOffset;
            }
    ...
    Sprite_Picture.prototype.makeDynamicBitmap = function () {
        ...
        var pad_h = pad;
        if (this.dTextInfo.value.contains("㊦")) {
            pad_h -= this.dTextInfo.size / 2.6;
        }
        this.bitmap = new Bitmap(bitmapVirtual.width + pad, bitmapVirtual.height + pad_h);
    ...
    Sprite_Picture.prototype._processCharacter = function (textState, bitmap) {
        if (textState.text[textState.index] === '\x1b') {
            ...
            var c = textState.text[textState.index++];
        } else if (textState.text[textState.index] === '[' || textState.text[textState.index] === '┏') {
            this._startMiniText(textState, bitmap);
            var c = textState.text[textState.index++];
        } else if (textState.text[textState.index] === ']' || textState.text[textState.index] === '┓') {
    ...
    Sprite_Picture.prototype._startRuby = function (textState) {
        textState.inRuby = true;
        textState.RubyStart_x = textState.x;
        textState.RubyStart_y = textState.y + 10;
        textState.RubyCount = 0;
    };

    Sprite_Picture.prototype._endRuby = function (textState) {
        textState.inRuby = false;
        textState.RubyEnd_x = textState.x;
        textState.RubyEnd_y = textState.y;
    };

    Sprite_Picture.prototype._startMiniText = function (textState) {
        textState.inMini = true;
    };

    Sprite_Picture.prototype._endMiniText = function (textState) {
    ...
    Sprite_Picture.prototype._RubyTextAdd = function (textState, textToAdd) {
        textState.inRubyTextString += textToAdd;
    };

    Sprite_Picture.prototype._RubyTextDelete = function (textState) {
        textState.inRubyText = "";
    };

    Sprite_Picture.prototype._endRubyText = function (textState, bitmap) {
        var w = this.hiddenWindow.textWidth(textState.inRubyTextString);
        var offset = -1 * (bitmap.fontSize / 2 + bitmap.fontSize / 2.6);
        if (bitmap.fontSize <= 100) {
            offset = -1 * bitmap.fontSize;
        }
        var originalFontSize = bitmap.fontSize;
        bitmap.fontSize = originalFontSize / 2.6;
        var font_mini = originalFontSize / 5.2;
        var center = (textState.RubyEnd_x + textState.RubyStart_x) / 2;
        if (bitmap.outlineWidth >= 50) {
            const regex = /[ａ-ｚＡ-Ｚ▮]+/g;
            const matches = textState.inRubyTextString.match(regex);
            if (matches) {
                ...
                const result_reverse = matches.map((match) => {
                    if (match.includes('▮')) {
                        return `${'　'.repeat(match.length)}`;
                    } else {
                        return `${match}`;
    ...
    Sprite_Picture.prototype._processNewLine = function (textState, bitmap) {
        ...
        if (this.dTextInfo.isRuby && this.dTextInfo.align == 0) {
            if (this.dTextInfo.size >= 100) {
